// apps/web/src/app/contact/page.tsx
// PROJECT OBSIDIAN — Contact Page

export default function ContactPage() {
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      <div className="mb-12 text-center">
        <span className="px-4 py-1.5 rounded-full bg-primary/10 border border-primary/20 text-primary font-mono text-xs font-bold tracking-widest mb-6 inline-block">
          SECURE COMM CHANNEL
        </span>
        <h1 className="text-4xl font-black tracking-tight text-text font-grotesk uppercase drop-shadow-lg">
          Transmit <span className="text-primary">Signal</span>
        </h1>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-5 gap-8">
        
        {/* ═══ CONTACT INFO ═══ */}
        <div className="md:col-span-2 space-y-6">
          <div className="glass-panel p-6">
            <h3 className="text-sm font-mono font-bold text-primary mb-1">TERMINAL LOCATION</h3>
            <p className="text-text font-grotesk text-lg">Sector 04 HQ</p>
            <p className="text-textMuted font-mono text-xs mt-2">
              Technopolis Park<br />
              Rabat 11100<br />
              Morocco
            </p>
          </div>

          <div className="glass-panel p-6">
            <h3 className="text-sm font-mono font-bold text-primary mb-1">DIRECT LINK</h3>
            <p className="text-text font-grotesk text-lg">+212 537 000 000</p>
            <p className="text-textMuted font-mono text-xs mt-2">
              ops@aurora-terminal.local
            </p>
          </div>

          <div className="glass-panel p-6 bg-primary/5 border-primary/20">
            <div className="flex items-center gap-3 font-mono text-xs font-bold">
              <span className="h-2 w-2 rounded-full bg-emerald-400 animate-pulse" />
              <span className="text-emerald-400">OPERATORS STANDING BY</span>
            </div>
          </div>
        </div>

        {/* ═══ SECURE FORM ═══ */}
        <div className="md:col-span-3 glass-panel p-8">
          <form className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                <label className="block text-xs font-mono font-bold text-textDim mb-2">OPERATIVE NAME</label>
                <input 
                  type="text" 
                  className="w-full bg-slate-950 border border-surfaceBorder rounded-lg px-4 py-3 text-sm font-mono text-text focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/50 transition"
                  placeholder="Enter alias"
                />
              </div>
              <div>
                <label className="block text-xs font-mono font-bold text-textDim mb-2">SECURE EMAIL</label>
                <input 
                  type="email" 
                  className="w-full bg-slate-950 border border-surfaceBorder rounded-lg px-4 py-3 text-sm font-mono text-text focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/50 transition"
                  placeholder="alias@domain.com"
                />
              </div>
            </div>

            <div>
              <label className="block text-xs font-mono font-bold text-textDim mb-2">TRANSMISSION CLASSIFICATION</label>
              <select className="w-full bg-slate-950 border border-surfaceBorder rounded-lg px-4 py-3 text-sm font-mono text-text focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/50 transition appearance-none">
                <option>Asset Inquiry</option>
                <option>Technical Support</option>
                <option>Partnership</option>
              </select>
            </div>

            <div>
              <label className="block text-xs font-mono font-bold text-textDim mb-2">ENCRYPTED PAYLOAD</label>
              <textarea 
                rows={5}
                className="w-full bg-slate-950 border border-surfaceBorder rounded-lg px-4 py-3 text-sm font-mono text-text focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/50 transition resize-none"
                placeholder="Enter message data..."
              />
            </div>

            <button type="button" className="w-full relative group overflow-hidden rounded-xl bg-slate-950 p-px font-mono text-sm font-bold text-primary shadow-[0_0_15px_rgba(34,211,238,0.15)] active:scale-[0.99] transition mt-4">
              <span className="absolute inset-0 bg-gradient-to-r from-primary/40 via-primary to-primary/40 opacity-50 group-hover:opacity-100 transition duration-500" />
              <span className="relative flex w-full items-center justify-center gap-3 rounded-[11px] bg-slate-900/95 px-8 py-4 backdrop-blur-xl transition group-hover:bg-slate-900/80">
                <span>TRANSMIT SIGNAL</span>
                <svg className="w-4 h-4 group-hover:translate-x-1 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M14 5l7 7m0 0l-7 7m7-7H3" />
                </svg>
              </span>
            </button>
          </form>
        </div>

      </div>
    </div>
  );
}
