  ...
  if (inventory.length === 0) {
    return (
      <div className="col-span-full glass-panel p-12 text-center">
        <p className="text-amber-500 font-mono text-sm">[NO_ASSETS_FOUND]: Adjust filter parameters.</p>
      </div>
    );
  }
            ...
            <Link href={`/inventory/${vehicle.id}`} className="block group h-full">
              <div className="glass-panel overflow-hidden flex flex-col justify-between group-hover:border-primary/40 group-hover:shadow-cyan-glow transition-all duration-300 h-full">
                ...
                <div className="p-5">
                  ...
                  <h2 className="text-base font-bold text-text font-grotesk group-hover:text-primary transition">
                    {vehicle.year} {vehicle.make} {vehicle.model}
                  </h2>
                  <p className="text-textMuted text-xs mt-1 capitalize font-mono">
                    {vehicle.exteriorColor} • {vehicle.mileage.toLocaleString('en-US')} km
                  </p>
                </div>

                {/* Price Bar */}
                <div className="px-5 py-3 border-t border-surfaceBorder flex items-center justify-between bg-surface/50 mt-auto">
                  <span className="text-lg font-bold text-primary font-grotesk">
                    MAD {Number(vehicle.price).toLocaleString('en-US')}
                  </span>
                  <span className="text-[10px] font-mono font-semibold text-primary/70 group-hover:text-primary transition flex items-center gap-1">
                    SCAN ASSET
